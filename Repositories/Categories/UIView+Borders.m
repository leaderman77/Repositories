...
-(UIView*)getViewBackedOneSidedBorderWithFrame:(CGRect)frame andColor:(UIColor*)color
{
    ...
    [border setBackgroundColor:color];
    return border;
} //2x + 2 padding + center = width 

-(void)addCenterHorizontalseparatorWithHeight:(CGFloat)height color:(UIColor*)color leftOffset:(CGFloat)leftOffset topCenterWidth:(CGFloat)centerWidth andRightOffset:(CGFloat)rightOffset {
    CGFloat top = (self.height - height) / 2.0;
    CGFloat width = (self.width - (centerWidth + leftOffset + rightOffset)) / 2.0 + rightOffset + centerWidth ;
    [self addTopBorderWithHeight:height color:color leftOffset:leftOffset rightOffset:width andTopOffset:top];
    [self addBottomBorderWithHeight:height color:color leftOffset:width rightOffset:rightOffset andBottomOffset:top];
