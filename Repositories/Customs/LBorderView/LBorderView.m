...
- (void)setShowChartingBorder:(BOOL)showChartingBorder {
    if(showChartingBorder) {
        self.selected = YES;
        self.borderWidth = 2.f;
        [self viewWithTag:3].backgroundColor = [UIColor clearColor];
    } else {
       self.selected = NO;
       self.borderWidth = 0;
       [self viewWithTag:3].backgroundColor = rgbColor(237, 237, 237);

    }
    _showChartingBorder = showChartingBorder;
}
